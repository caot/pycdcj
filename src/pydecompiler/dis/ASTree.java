...
public class ASTree {
  ...
  static ASTNode BuildFromCode(PycCode code, PycModule mod) throws IOException {
      ...
      Deque<FastStack> stack_hist = new ArrayDeque<FastStack>();

      Deque<ASTBlock> blocks = new ArrayDeque<ASTBlock>();
  ...
  if (BLOCK_DEBUG || STACK_DEBUG) {
      ...
      if (BLOCK_DEBUG) {
          ...
          curpos = pos;
          Args args = bytecode.bc_next(source, code, mod, opcode, operand, pos, is_disasm);
      ...
      // mod = args.mod;
          ...
          if (need_try && opcode != Pyc.Opcode.SETUP_EXCEPT_A) {
              ...
              ASTBlock tryblock = new ASTBlock(ASTBlock.BlkType.BLK_TRY, curblock.end(), Boolean.TRUE);
              blocks.push(tryblock);
              curblock = blocks.peek(); //.peek();
          } else if (else_pop
              ...
              while (prev.end() < pos
                  ...
                  if (prev.blktype() != ASTBlock.BlkType.BLK_CONTAINER) {
                      ...
                      //stack = stack_hist.peek();
                      if (!stack_hist.isEmpty())
                          stack_hist.pop();
                  }
                  blocks.pop();
          ...
          case BINARY_SUBSCR:
              {
                  ASTNode subscr = stack.peek();
                  stack.pop();
                  ASTNode src = stack.peek();
                  stack.pop();
          ...
          case BUILD_CLASS:
              {
                  ASTNode _code = stack.peek();
                  stack.pop();
                  ASTNode bases = stack.peek();
                  stack.pop();
                  ASTNode name = stack.peek();
                  stack.pop();
                  stack.push(new ASTClass(_code, bases, name));
              }
              break;
          case BUILD_FUNCTION:
              {
                  ASTNode _code = stack.peek();
                  stack.pop();
                  stack.push(new ASTFunction(_code, new LinkedList<ASTNode>()));
              }
              break;
          case BUILD_LIST_A:
              {
        LinkedList<ASTNode> values = new LinkedList<ASTNode>(); // /?????????????????????
          ...
          case STORE_MAP:
              {
                  List<ASTNode> values;
                  ASTNode key = stack.peek();
                  stack.pop();
                  ASTNode value = stack.peek();
                  stack.pop();
                  ASTMap map = ((ASTMap)stack.peek());
                  map.add(key, value);
              }
              break;
          case BUILD_SLICE_A:
              {
                  if (operand == 2) {
                      ASTNode end = stack.peek();
                      stack.pop();
                      ASTNode start = stack.peek();
                      stack.pop();
        ...
        LinkedList<ASTNode> pparamList = new LinkedList<ASTNode>(); // ?????????????????????
                  ...
                  for (int i=0; i<pparams; i++) {
                      pparamList.addFirst(stack.peek());
                      stack.pop();
                  }
                  ASTNode func = stack.peek();
                  stack.pop();
        ...
        // if (stack.peek() != null)
          ...
          case DUP_TOP_TWO:
              {
                  ASTNode first = stack.peek();
                  stack.pop();
                  ASTNode second = stack.peek();
          ...
          case DUP_TOPX_A:
              {
                  Deque<ASTNode> first = new ArrayDeque<ASTNode>();
                  Deque<ASTNode> second = new ArrayDeque<ASTNode>();
                  ...
                  while (!first.isEmpty()) {
                      stack.push(first.peek());
                      first.pop();
                  }

                  while (!second.isEmpty()) {
                      stack.push(second.peek());
                      second.pop();
                  }
              }
              break;
          case END_FINALLY:
              {
                  ...
                  if (curblock.blktype() == ASTBlock.BlkType.BLK_FINALLY) {
                      ...
                      ASTBlock prev = curblock;
                      if (curblock.size() != 0) {
                          blocks.peek().append((ASTNode)curblock);
                      }
          ...
          case EXEC_STMT:
              {
                  ASTNode loc = stack.peek();
                  stack.pop();
                  ASTNode glob = stack.peek();
                  stack.pop();
          ...
          case FOR_ITER_A:
              {
                  ASTNode iter = stack.peek(); // Iterable
                  stack.pop();
                  /* Pop it? Don't pop it? */

                  boolean comprehension = false;
                  ASTBlock top = blocks.peek();
          ...
          case FOR_LOOP_A:
              {
                  ASTNode curidx = stack.peek(); // Current index
                  stack.pop();
                  ASTNode iter = stack.peek(); // Iterable
                  stack.pop();
          ...
          case POP_JUMP_IF_TRUE_A:
              {
                  ...
                  if (cond.type() == ASTNode.Type.NODE_COMPARE
                  ...
                  } else if (curblock.size() == 0 && curblock.inited() == null
                      ...
                      if (curblock.blktype() == ASTBlock.BlkType.BLK_WHILE) {
                          ...
                          FastStack s_top = stack_hist.peek();
                          stack_hist.pop();
                          stack_hist.pop();
          ...
          case JUMP_ABSOLUTE_A:
              {
                  ...
                  do {
                      ...
                      if (blocks.isEmpty())
                          prev = nil;
                      blocks.peek().append((ASTNode)prev);
                      ...
                      if (prev.blktype() == ASTBlock.BlkType.BLK_IF
                          ...
                          ASTBlock next = new ASTBlock(ASTBlock.BlkType.BLK_ELSE, blocks.peek().end());
                          if (prev.inited() == ASTCondBlock.InitCond.PRE_POPPED) {
                          ...
                          ASTBlock next = new ASTCondBlock(ASTBlock.BlkType.BLK_EXCEPT, blocks.peek().end(), ASTNode.Node_NULL, false);
                          next.init();
          ...
          case JUMP_FORWARD_A:
              {
                  ...
                  do {
                      ...
                      if (prev.blktype() == ASTBlock.BlkType.BLK_IF
                          ...
                          if (blocks.peek().blktype() == ASTBlock.BlkType.BLK_CONTAINER) {
                              ASTContainerBlock cont = (ASTContainerBlock)(blocks.peek());
                              if (cont.hasExcept()) {
                                  if (push) {
                                      stack_hist.push(stack);
                                  }
          ...
          case POP_TOP:
              {
                  ASTNode value = stack.peek();
                  stack.pop();
                  if (curblock.inited() == null) {          ///???????????********************
                      if (curblock.blktype() == ASTBlock.BlkType.BLK_WITH) {
                          ((ASTWithBlock)curblock).setExpr(value);
                      } else {
//                        if (curblock instanceof ASTCondBlock)
                      ...
                      break;
                  } else if (value.type() == ASTNode.Type.NODE_COMPARE
              ...
              break;
          case PRINT_ITEM:
              curblock.append(new ASTPrint(stack.peek()));
              stack.pop();
              break;
          case PRINT_ITEM_TO:
              {
                  ASTNode stream = stack.peek();
                  stack.pop();

                  curblock.append(new ASTPrint(stack.peek(), stream));
                  stack.pop();
                  break;
              }
          case PRINT_NEWLINE:
          ...
          case PRINT_NEWLINE_TO:
              curblock.append(new ASTPrint(ASTNode.Node_NULL, stack.peek()));
              stack.pop();
              break;
          case RAISE_VARARGS_A:
              {
                LinkedList<ASTNode> paramList = new LinkedList<ASTNode>();
                  ...
                  if ((curblock.blktype() == ASTBlock.BlkType.BLK_IF
                      ...
                      args = bytecode.bc_next(source, code, mod, opcode, operand, pos, is_disasm);
                      opcode = args.opcode;
                      operand = args.operand;
                      pos = args.pos;
                  }
          ...
          case ROT_TWO:
              {
                  ASTNode one = stack.peek();
                  stack.pop();
                  ASTNode two = stack.peek();
                  stack.pop();
          ...
          case STORE_FAST_A:
              {
                  if (unpack > 0) {
                      ...
                      } else if (stack.peek().type() == ASTNode.Type.NODE_IMPORT) {
                          ASTImport import_ = (ASTImport)stack.peek();
          ...
          case STORE_GLOBAL_A:
              {
                ASTNode name = new ASTName(code.getName(operand));
                  ...
                  if (unpack > 0) {
                      ...
                      if (((ASTName)name).name() == ((ASTName)value).name()) {
                          ASTImport import_ = (ASTImport)(stack.peek());
                          import_.add_store(new ASTStore(value, name));
                      } else {
                          curblock.append(new ASTStore(value, name));
          ...
          case YIELD_VALUE:
              ...
              break;
          default:
              System.err.printf("Unsupported opcode: %s\n", Pyc.OpcodeName(opcode));
              cleanBuild = false;
      ...
      System.err.printf("Warning: Stack history is not empty!\n");
      ...
      if (blocks.size() > 1) {
      System.err.printf("Warning: block stack is not empty!\n");

          while (blocks.size() > 1) {
              ASTBlock tmp = blocks.peek();
              blocks.pop();

              blocks.peek().append(tmp);
          }
      }
  ...
  static void print_ordered(ASTNode parent, ASTNode child,
                            PycModule mod) throws IOException
  {
      if (child.type() == ASTNode.Type.NODE_BINARY ||
  ...
  static void print_src(ASTNode node, PycModule mod) throws IOException
  {
      ...
      case NODE_COMPARE:
          {
      // ASTCompare bin = (ASTCompare) node;
      ASTBinary bin = (ASTBinary) node;
              print_ordered(node, bin.left(), mod);
              pyc_output.printf("%s", bin.op_str());
              print_ordered(node, bin.right(), mod);
      ...
      case NODE_CALL:
          {
              ...
              for (Iterator<Pair<ASTNode, ASTNode >> p = call.kwparams().iterator(); p.hasNext(); ) {
                  ...
                  pyc_output.printf("%s = ", ((ASTName)next.getFirst()).name().value());
                  print_src((ASTName)next.getSecond(), mod);
                  first = false;
              }
              if (call.hasVar()) {
                  if (!first)
      ...
      case NODE_MAP:
          {
            List<Pair<ASTNode, ASTNode>> values = ((ASTMap)node).values();
              ...
              for (Iterator<Pair<ASTNode, ASTNode>> b = values.iterator(); b.hasNext(); ) {
                  ...
                  start_line(cur_indent);
                  print_src(next.getFirst(), mod);
                  pyc_output.printf(": ");
                  print_src(next.getSecond(), mod);
                  first = false;
              }
              cur_indent--;
              pyc_output.printf(" }");
...
//                    ASTTuple bases = (ASTTuple)((ASTClass)src).bases();
                  ...
                  if (bases instanceof ASTTuple)
                    ...
                    System.err.print("        bases class name: " + bases.getClass().getName());
                  if (values.size() > 0) {
                      pyc_output.printf("(");
                      boolean first = true;
                      for (Iterator<ASTNode> b = values.iterator(); b.hasNext(); ) {
                  ...
                  ASTNode code = ((ASTFunction)((ASTCall)((ASTClass)src).code())
                                         .func()).code();
                  print_src(code, mod);
